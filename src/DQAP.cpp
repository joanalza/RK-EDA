...
int DQAP::ReadInstance(string filename)
{
 ...
 */
double DQAP::Evaluate(int * genes)
{
	double fitness=0;
	int FactA, FactB;
	...
	for (i=0;i<m_size;i++)
	{
		for (j=0;j<m_size;j++)
		{
			FactA = genes[m_identityPerm[i]];
			FactB = genes[m_identityPerm[j]];
			
			distAB= m_distance_matrix[m_identityPerm[i]][m_identityPerm[j]];
 ...
 */
double DQAP::EvaluateInv(int * genes)
{
