...
double DTSP::CalculateGEODistance(double latitudeX, double latitudeY, double longitudeX, double longitudeY)
{
	double PI = 3.141592;
	double RRR = 6378.388;
 ...
 */
int DTSP::ReadTSPLIB(string filename)
{
	...
	while (!indata.eof())
	{
		...
		if(Tools::strContains(sline,"DIMENSION"))
		{
			char * pch;
			pch = strtok (line," ");
			pch = strtok (NULL, " ");
		...
		else if (Tools::strContains(sline,"EDGE_WEIGHT_TYPE"))
		{
			char * pch;
			pch = strtok (line," ");
			pch = strtok (NULL, " ");
 ...
 */
int DTSP::ReadInstance(string filename)
{
	char line[2048]; // variable for input value
	string data="";
	ifstream indata;
 ...
 */
double DTSP::Evaluate(int * genes)
{
	...
	for (int i=0;i<m_size;i++)
	{
		IDCityA = genes[m_identityPerm[i]];
		IDCityB = genes[m_identityPerm[0]];
		if (i+1<m_size)
		{
			IDCityB = genes[m_identityPerm[i+1]];
		}
 ...
 */
double DTSP::EvaluateInv(int * genes){
    Tools::Invert(genes, m_size, m_aux);
	...
	for (int i=0;i<m_size;i++)
	{
		IDCityA = m_aux[m_identityPerm[i]];
		IDCityB = m_aux[m_identityPerm[0]];
		if (i+1<m_size)
		{
			IDCityB = m_aux[m_identityPerm[i+1]];
		}
